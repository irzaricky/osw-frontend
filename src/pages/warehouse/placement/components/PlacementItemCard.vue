...
<template>
  <div class="rounded-xl border border-default p-4 space-y-3">
    <div class="flex items-start justify-between gap-4">
      <div>
        <p class="font-semibold">{{ item.part_number }}</p>
        <p class="text-sm text-muted">{{ item.part_name }}</p>
        <p class="text-xs text-muted">{{ item.part_category || '-' }}</p>
    ...
    <PlacementProgress :total="item.total_label" :scanned="item.total_scanned" :progress="item.progress"
      :total-pcs="item.total_pcs" :scanned-pcs="item.scanned_pcs" />

    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
      <div v-for="label in item.labels" :key="label.wo_item_label_id"
