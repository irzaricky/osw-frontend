...
export function useCustomerColumns(actions: Actions, ui: UIComponents) {
  const getActionItems = (row: Row<Customer>) => [
    [
      {
        label: 'Edit',
  ...
  const columns: ColumnDef<Customer>[] = [
    ...
    },
    {
      ...
      header: 'Code',
      cell: ({ row }) => h('div', { class: 'font-medium' }, row.original.customer_code)
    },
    {
      accessorKey: 'name',
      header: 'Name',
    ...
    },
    {
      accessorKey: 'address',
      header: 'Address',
      cell: ({ row }) => h('div', { class: 'truncate max-w-[300px] text-muted' }, row.original.address)
    },
    {
      id: 'actions',
      header: '',
