...
// ─── Calendar offset (align day-1 to correct weekday column) ─────────────────
...
const allShiftGroups = computed(() => {
  ...
  // Fallback: if base_shifts are empty (e.g. all days uninitialized),
  // include shifts 1–3 with generic names so the modal still works
  if (map.size === 0) {
    map.set(1, 'Shift 1')
    map.set(2, 'Shift 2')
    map.set(3, 'Shift 3')
...
// Existing overtime grouped by shift_number for a given day
...
// How many new shift slots are still available
...
// How many new shifts the user has currently checked
const newShiftsSelected = computed(() =>
  dayModal.shiftInputs.filter(s => !s.isAlreadyActive && s.addShift).length
)

// Form is valid when there is at least one meaningful action and a reason
...
function handleSubmit() {
  ...
  const shifts = dayModal.shiftInputs
    .filter(s => (!s.isAlreadyActive && s.addShift) ||
                 ((s.isAlreadyActive || s.addShift) && s.overtimeMinutes != null && s.overtimeMinutes > 0))
    .map(s => ({
      shift_number:     s.shift_number,
      overtime_minutes: (s.isAlreadyActive || s.addShift) && s.overtimeMinutes != null && s.overtimeMinutes > 0
        ? s.overtimeMinutes
        : null,
    }))
...
// Max remaining OT for the shift being edited (excluding this record itself)
          ...
          <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
            <div
              v-for="metric in [
                ...
                { label: 'Gap',            value: `${result.capacity_gap_units >= 0 ? '+' : ''}${fmtNum(result.capacity_gap_units)} unit`, icon: 'i-lucide-package', sub: 'vs demand' },
    ...
    >
      <template v-if="dayModal.day" #body>
        <div class="space-y-4">
          ...
          <!-- Existing adjustments (with edit for OT, delete for all) -->
          <div v-if="dayModal.day.adjustments.length > 0">
            ...
            <div class="space-y-1.5">
              <div
                v-for="adj in dayModal.day.adjustments"
                :key="adj.id"
          ...
          <!-- Add new shifts / overtime section -->
          <template v-if="isEditable">
            <div class="border-t border-default pt-4">
              ...
              <div class="space-y-2">
                ...
                >
                  ...
                  <!-- Overtime input: show when shift is active or will be added -->
                  <div
