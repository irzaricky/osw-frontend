...
import type { BomDetail, BomDetailType, BomDropdownItem } from '../../../../types/production-plan/bom'
...
// ─── Type options ─────────────────────────────────────────────────────────────
const DETAIL_TYPE_OPTIONS: { label: string; value: BomDetailType }[] = [
...
const type             = ref<BomDetailType | null>(null)
...
watch(
  ...
  (val) => {
    ...
    if (props.mode === 'edit' && props.detail) {
      ...
      type.value             = (d.type as BomDetailType) ?? null
...
const uomItems = computed(() =>
  props.uomDropdown.map((u) => ({ label: `${u.code} — ${u.name}`, value: u.id })),
)
const selectedUomId = computed({
  get: () => uom_id.value ?? undefined,
  set: (v: number | null) => { uom_id.value = v ?? undefined },
})
...
const selectedChildBomId = computed({
  ...
  set: (v: number | null) => { child_bom_id.value = v ?? undefined },
})

// ─── Type select ──────────────────────────────────────────────────────────────
const selectedType = computed({
  get: () => type.value ?? undefined,
  set: (v: BomDetailType | null) => { type.value = v ?? null },
})

// ─── Submit ───────────────────────────────────────────────────────────────────
function handleSave() {
  const uomEntry    = props.uomDropdown.find((u) => u.id === uom_id.value)
  const childBomEntry = props.bomDropdown.find((b) => b.id === child_bom_id.value)
  ...
  emit('save', {
    ...
    _part: selectedPart.value
      ? { id: selectedPart.value.id, part_number: selectedPart.value.part_number, part_name: selectedPart.value.part_name }
      : undefined,
    _uom: uomEntry
      ? { id: uomEntry.id, code: uomEntry.code, name: uomEntry.name }
      : undefined,
    _child_bom: childBomEntry
      ? { id: childBomEntry.id, bom_number: childBomEntry.bom_number, bom_version: childBomEntry.bom_version }
      : undefined,
  })
}
</script>
      ...
      <form id="detail-form" class="space-y-4" @submit.prevent="handleSave">
        ...
        <UFormField label="Component Part" required>
          <div class="space-y-2">
            ...
            >
              <div class="text-sm">
                <span class="font-mono font-semibold text-xs">{{ selectedPart.part_number }}</span>
                <span class="mx-2 text-muted">·</span>
                <span>{{ selectedPart.part_name }}</span>
                <span v-if="selectedPart.uom" class="ml-2 text-xs text-muted">({{ selectedPart.uom.code }})</span>
              </div>
              <UButton
        ...
        <UFormField label="Child BOM (optional)">
          <template v-if="!selectedPart">
            <p class="text-xs text-muted py-1.5">Select a component part first to see available child BOMs.</p>
          </template>
