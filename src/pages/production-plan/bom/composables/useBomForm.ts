...
export function useBomForm() {
  ...
  const { currentBom, loading, saving, dropdown: bomDropdown } = storeToRefs(bomStore)
  ...
  // ─── Doc status ─────────────────────────────────────────────────────────────
  function docStatusColor(code?: string): 'neutral' | 'warning' | 'success' | 'error' {
    switch (code) {
  ...
  const uomItems = computed(() =>
    uomDropdown.value.map((u) => ({ label: `${u.code} — ${u.name}`, value: u.id })),
  )

  const selectedUomId = computed({
    get: () => headerForm.uom_id ?? undefined,
    set: (v: number | null) => { headerForm.uom_id = v ?? undefined },
  })
  ...
  function syncDetailsFromBom() {
    if (!currentBom.value?.details) { localDetails.value = []; return }
    localDetails.value = currentBom.value.details.map((d) => {
      ...
      return {
        ...
        type:             (d.type as BomDetailType) ?? null,
        notes:            d.notes ?? null,
  ...
  function handleDetailSave(data: Partial<BomDetail> & {
    ...
    if (detailModalMode.value === 'add') {
      localDetails.value.push({
        ...
        type:             (data.type as BomDetailType) ?? null,
      ...
      const idx = localDetails.value.findIndex((d) => d.temp_id === editingDetail.value!.temp_id)
      if (idx !== -1) {
  ...
  function confirmDeleteDetail(d: LocalDetail) {
    deleteConfirm.description = `Remove "${d._part?.part_name ?? `Part #${d.part_id}`}" from this BOM?`
  ...
  function executeDeleteDetail() {
    ...
    localDetails.value   = localDetails.value.filter((d) => d.temp_id !== deleteConfirm.target!.temp_id)
  ...
  function handleProceedToDetails() {
    if (!selectedParentPart.value) { toastError('Please select a parent part.'); return }
    createStep.value = 2
  }
  ...
  async function handleSave(andSubmit = false) {
    ...
    try {
      ...
      if (isCreate.value) {
        ...
        savedId = res.data.id
        if (andSubmit) { await bomStore.submit(savedId); toastSuccess('BOM submitted for approval') }
  ...
  function openApproveConfirm() {
    Object.assign(wfModal, {
      ...
      inputLabel: '', inputRequired: false, inputValue: '',
    })
    wfModal.action = async () => {
      ...
      finally { wfModal.actionLoading = false }
    }
    wfModal.open = true
  }
  ...
  function openRejectConfirm() {
    Object.assign(wfModal, {
      ...
      inputLabel: 'Rejection Reason', inputRequired: true, inputValue: '',
    })
    wfModal.action = async () => {
      if (!wfModal.inputValue.trim()) { toastError('Rejection reason is required.'); return }
      try {
        wfModal.actionLoading = true
        const res = await bomStore.reject(bomId.value!, { reject_reason: wfModal.inputValue })
  ...
  function openActivationConfirm() {
    ...
    Object.assign(wfModal, {
      ...
      inputLabel: '', inputRequired: false,
    })
    wfModal.action = async () => {
      try {
        wfModal.actionLoading = true
        ...
        toastSuccess(res.message || (isActive ? 'BOM deactivated' : 'BOM activated'))
        wfModal.open = false
      } catch (e) { toastError(e) }
      finally { wfModal.actionLoading = false }
    }
  ...
  watch(currentBom, (bom) => {
    if (!bom || isCreate.value) return
    headerForm.description = bom.description ?? ''
    headerForm.notes       = bom.notes ?? ''
    if (bom.uom) {
      headerForm.uom_id = uomDropdown.value.find((u) => u.id === bom.uom!.id)?.id ?? bom.uom.id
    }
    syncDetailsFromBom()
    savedSnapshot.value = makeSnapshot()
  }, { deep: true })
  ...
  return {
    ...
    syncDetailsFromBom, discardChanges,
