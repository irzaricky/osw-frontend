...
export const useBomStore = defineStore("bom", () => {
   ...
   *
   * Update header + replace semua detail sekaligus:
   *   updateBom(id, { description: 'Revisi', details: [...] })
   *
   * Hanya replace detail ([] untuk hapus semua):
  ...
  /**
   * Bulk-replace semua detail (e.g. setelah drag-to-reorder).
   * Refresh otomatis setelah berhasil.
   */
  async function replaceDetails(
    id: number | string,
    payload: ReplaceDetailsPayload
  ) {
    saving.value = true;
    error.value = null;
    try {
      const { data } = await bomService.replaceDetails(id, payload);
      if (data.status) await fetchBom(id);
      return data;
    } catch (e: any) {
      error.value = e.response?.data?.error || e.message;
  ...
  async function reject(id: number | string, payload: RejectBomPayload) {
    saving.value = true;
    error.value = null;
    try {
